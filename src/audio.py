...
`SpeechPipeline` starts the microphone and the recognizer together and hands
each transcript to a callback, producing the text `matcher.py` turns into
counts. It starts only once both authorizations are granted, and raises
OnDeviceUnavailable on a machine that cannot recognize speech without the
network.

`checks/check_audio.py` runs the pipeline on its own.
"""
...
# The audio engine's input node carries a single bus, numbered 0.
INPUT_BUS = 0
...
class SpeechPipeline:
    ...
    Built once and started and stopped as many times as there are sessions.
    ...
    recognition was given up on. Both are called on whichever thread the
    recognizer delivers to; a caller that touches an interface is responsible
    for getting itself back to the main thread.

    `on_stalled` is called with the name of the input device when the
    microphone stops sending audio. Stopping the pipeline is left to whoever
    receives it.
    ...
    application running `NSApplication.run()` leaves this false; a script sets
    it true.
    """
    ...
    @property
    def buffers_appended(self):
        """How many audio buffers the tap has passed to a request."""
        return self._buffers

    @property
    def tasks_started(self):
        """How many recognition tasks have been started, the restarts included."""
    ...
    def stop(self):
        ...
        Unwound in the order the engine expects. A pipeline that is already
        stopped is left as it stands, so this is safe to call more than once.
        """
        self._live_task_number = None
    ...
    def _restart(self):
        ...
        A pipeline that is already stopped is left as it stands.
        """
    ...
    def _restarts_are_too_fast(self):
        ...
        The window opens on the first restart after each gap of
    ...
    def pump(self, seconds):
        ...
        once the pipeline has been stopped, and returns straight away unless
        the pipeline was built with pump_run_loop.
        """
    ...
    def _retire_stall_timer(self):
        """Stop reading the buffer count, where a timer is running."""
